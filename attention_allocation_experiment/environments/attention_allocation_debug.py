...
def _update_state(state, incidents_occurred, incidents_reported, action, alpha=0.0):
    ...
    # Handle dynamics.
    for location_ind in range(params.n_locations):
        # factor = - alpha * max(params.incident_rates[location_ind]-5, 0)
        ...
        if attention == 0:
            ...
            params.incident_rates[location_ind] = max(
                0.0, params.incident_rates[location_ind] -
                     (params.dynamic_rate[1][location_ind] * attention))
...
class LocationAllocationEnv(core.FairnessEnv):
    ...
    def _step_impl(self, state, action):
        ...
        _update_state(state, incidents_occurred, reported_incidents, action, self.alpha)
        return state
