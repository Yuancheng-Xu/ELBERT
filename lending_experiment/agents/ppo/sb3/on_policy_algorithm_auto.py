...
from lending_experiment.environments.lending_params import DelayedImpactParams
...
from lending_experiment.agents.ppo.sb3.utils_auto import evaluate_auto

class OnPolicyAlgorithm_auto(BaseAlgorithm):
    """
    ...
    def __init__(
        ...
        # for auto eval
        eval_env_params: DelayedImpactParams = None,
    ...
    def learn(
        ...
        tb_log_name: str = "OnPolicyAlgorithm_auto",
        eval_log_path: Optional[str] = None,
        reset_num_timesteps: bool = True,
    ) -> "OnPolicyAlgorithm_auto":
        iteration = 0
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.eval_interval is not None and (iteration) % self.eval_interval == 0:
                self.policy.set_training_mode(False)
                # env_eval = GeneralDelayedImpactEnv(self.eval_env_params) # for setting env params
                if 'General' in str(type(self.env.envs[0].env.env)):
                    ...
                    # env_eval = GeneralDelayedImpactEnv(self.eval_env_params)
                else:
                    print('Evaluation: Using Original Eric\'s modified env')
                    env_eval = DelayedImpactEnv(self.eval_env_params)
                # may need fix
                env_eval = PPOEnvWrapper_auto(env=env_eval, reward_fn=LendingReward, name=self.name, ep_timesteps=self.eval_timesteps)
                ...
                name = "train_%d" % (iteration*self.n_steps)
                Path(f'{self.eval_write_path}/{name}/').mkdir(parents=True, exist_ok=True)
                for key in eval_data.keys():
                    np.save(f'{self.eval_write_path}/{name}/{key}.npy', eval_data[key])
